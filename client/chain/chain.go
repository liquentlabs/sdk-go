...
import (
	...
	exchangetypes "github.com/liquentlabs/sdk-go/chain/exchange/types"
	permissionstypes "github.com/liquentlabs/sdk-go/chain/permissions/types"
	chainstreamtypes "github.com/liquentlabs/sdk-go/chain/stream/types"
	tokenfactorytypes "github.com/liquentlabs/sdk-go/chain/tokenfactory/types"
...
const (
	...
	SpotOrderbook                    = "liquent.exchange.v1beta1.EventOrderbookUpdate.spot_orderbooks"
	DerivativeOrderbook              = "liquent.exchange.v1beta1.EventOrderbookUpdate.derivative_orderbooks"
)
...
func (c *chainClient) BuildSignedTx(clientCtx sdkclient.Context, accNum, accSeq, initialGas uint64, gasPrice uint64, msgs ...sdk.Msg) ([]byte, error) {
	...
	gasPriceWithDenom := fmt.Sprintf("%d%s", gasPrice, client.LqtDenom)
...
func (c *chainClient) GetGasFee() (string, error) {
	gasPrices := strings.TrimSuffix(c.txFactory.GasPrices().String(), client.LqtDenom)
...
func (c *chainClient) StreamEventOrderFailWithWebsocket(sender string, websocket *rpchttp.HTTP, failEventCh chan map[string]uint) {
	filter := fmt.Sprintf("tm.event='Tx' AND message.sender='%s' AND message.action='/liquent.exchange.v1beta1.MsgBatchUpdateOrders' AND liquent.exchange.v1beta1.EventOrderFail.flags EXISTS", sender)
	...
	for {
		...
		err = json.Unmarshal([]byte(e.Events["liquent.exchange.v1beta1.EventOrderFail.hashes"][0]), &failedOrderHashes)
		if err != nil {
			panic(err)
		}

		var failedOrderCodes []uint
		err = json.Unmarshal([]byte(e.Events["liquent.exchange.v1beta1.EventOrderFail.flags"][0]), &failedOrderCodes)
		if err != nil {
			panic(err)
		}
...
func (c *chainClient) SetGasPrice(gasPrice int64) {
	gasPrices := fmt.Sprintf("%v%s", gasPrice, client.LqtDenom)
