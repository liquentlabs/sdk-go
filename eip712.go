...
import (
	...
	lqtcodectypes "github.com/liquentlabs/sdk-go/chain/codec/types"
	secp256k1 "github.com/liquentlabs/sdk-go/chain/crypto/ethsecp256k1"
	chaintypes "github.com/liquentlabs/sdk-go/chain/types"
	"github.com/liquentlabs/sdk-go/typeddata"
)

var (
	chainTypesCodec codec.ProtoCodecMarshaler
	GlobalCdc       = codec.NewProtoCodec(lqtcodectypes.NewInterfaceRegistry())
)

func init() {
	registry := lqtcodectypes.NewInterfaceRegistry()
...
func GetWeb3ExtensionOptions(tx authsigning.Tx, signerData authsigning.SignerData) (Web3ExtensionOptions, error) {
	...
	// 1776 (Liquent EVM Mainnet), 1439 (Liquent EVM Testnet), thus Metamask will be
	// able to submit signatures without switching networks.
	hasValidMainnetChainID := signerData.ChainID == "liquent-1" && (extOpts.TypedDataChainID == 1 || extOpts.TypedDataChainID == 1776)
	hasValidNonMainnetChainID := (signerData.ChainID == "liquent-777" || signerData.ChainID == "liquent-888") && (extOpts.TypedDataChainID == 11155111 || extOpts.TypedDataChainID == 1439)
