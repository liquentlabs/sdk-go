...
const (
	...
	// SpotMarketInstantListingFee is 20 LQT
	SpotMarketInstantListingFee int64 = 20

	// DerivativeMarketInstantListingFee is 20 LQT
	DerivativeMarketInstantListingFee int64 = 20

	// BinaryOptionsMarketInstantListingFee is 100 LQT
...
var DefaultLqtAuctionMaxCap = math.NewIntWithDecimal(10_000, 18)
...
func (p *Params) ParamSetPairs() paramtypes.ParamSetPairs {
	return paramtypes.ParamSetPairs{
		...
		paramtypes.NewParamSetPair(
			KeyLqtRewardStakedRequirementThreshold, &p.LqtRewardStakedRequirementThreshold, ValidateLqtRewardStakedRequirementThreshold,
		),
		paramtypes.NewParamSetPair(
...
func (p Params) Validate() error {
	...
	if err := ValidateLqtRewardStakedRequirementThreshold(p.LqtRewardStakedRequirementThreshold); err != nil {
...
func ValidateLqtRewardStakedRequirementThreshold(i interface{}) error {
	v, ok := i.(math.Int)
	if !ok {
		return fmt.Errorf("invalid parameter type: %T", i)
	}
	...
	if v.IsZero() {
		return fmt.Errorf("LqtRewardStakedRequirementThreshold cannot be zero: %d", v)
	}

	if v.IsNegative() {
		return fmt.Errorf("LqtRewardStakedRequirementThreshold cannot be negative: %d", v)
	}
