...
import (
	...
	"github.com/liquentlabs/sdk-go/chain/exchange/types"
)
...
var (
	...
	KeyLqtRewardStakedRequirementThreshold         = []byte("KeyLqtRewardStakedRequirementThreshold")
...
func (p *Params) ParamSetPairs() paramtypes.ParamSetPairs {
	return paramtypes.ParamSetPairs{
		...
		paramtypes.NewParamSetPair(
			KeyLqtRewardStakedRequirementThreshold,
			&p.LqtRewardStakedRequirementThreshold,
			types.ValidateLqtRewardStakedRequirementThreshold,
		),
		paramtypes.NewParamSetPair(
...
func DefaultParams() Params {
	return Params{
		SpotMarketInstantListingFee:                  sdk.NewCoin("lqt", math.NewIntWithDecimal(types.SpotMarketInstantListingFee, 18)),
		DerivativeMarketInstantListingFee:            sdk.NewCoin("lqt", math.NewIntWithDecimal(types.DerivativeMarketInstantListingFee, 18)),
		...
		LqtRewardStakedRequirementThreshold:          math.NewIntWithDecimal(100, 18), // 100 LQT
		TradingRewardsVestingDuration:                604800,                          // 7 days
		LiquidatorRewardShareRate:                    math.LegacyNewDecWithPrec(5, 2), // 5% liquidator reward
		BinaryOptionsMarketInstantListingFee:         sdk.NewCoin("lqt", math.NewIntWithDecimal(types.BinaryOptionsMarketInstantListingFee, 18)),
		...
		LqtAuctionMaxCap:                             types.DefaultLqtAuctionMaxCap,
...
func (p Params) Validate() error {
	...
	if err := types.ValidateLqtRewardStakedRequirementThreshold(p.LqtRewardStakedRequirementThreshold); err != nil {
		return fmt.Errorf("lqt_reward_staked_requirement_threshold is incorrect: %w", err)
	}
