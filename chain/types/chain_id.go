...
var (
	...
	regexSeparator   = `-{1}`
	regexEpoch       = `[1-9][0-9]*`
	liquentChainID = regexp.MustCompile(fmt.Sprintf(`^(%s)%s(%s)$`, regexChainID, regexSeparator, regexEpoch))
)
...
func IsValidChainID(chainID string) bool {
	...
	return liquentChainID.MatchString(chainID)
}
...
func ParseChainID(chainID string) (*big.Int, error) {
	...
	matches := liquentChainID.FindStringSubmatch(chainID)
...
func GenerateRandomChainID() string {
	return fmt.Sprintf("liquent-%d", 10+rand.Intn(10000))
}
